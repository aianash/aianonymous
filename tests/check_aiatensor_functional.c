...
void functional_setup(void) {
  ...
  vectorT = aiatensor_(float, newFromData)(arr_(float, clone)(vector, 2), 1, size2, NULL);
}
...
START_TEST(test_vforeach) {
  ...
  endvforeach
  ck_assert_msg(count == 2, "Failed vforeach count = %ld", count);
}
END_TEST
...
START_TEST(test_vzip) {
  ...
  endvzip2
  ck_assert_msg(count == 2, "Failed vzip2 count = %ld", count);
  ...
  endvzip3
  ck_assert_msg(count == 2, "Failed vzip3 count = %ld", count);
}
END_TEST
...
START_TEST(test_vfor) {
  ...
  endvfor
  ck_assert_msg(count == 4, "Failed vfor count = %ld", count);
}
END_TEST
